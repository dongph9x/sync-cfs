...
async function upsertThread(thread: ThreadChannel): Promise<void> {
    ...
    try {
        ...
        if (starterMessage) {
            ...
            logger.debug({
                threadId: thread.id,
                originalContent: starterMessage.content,
                sanitizedContent: sanitizationResult.sanitizedContent,
                bodyHtmlLength: bodyHtml.length,
                hasImages: starterMessage.attachments.size > 0,
                isBotMessage: starterMessage.author.bot
            }, 'Processed starter message content');
                ...
                try {
                    ...
                    if (imageData.length > 0) {
                        ...
                        logger.debug({
                            ...
                            finalBodyHtmlLength: bodyHtml.length
                        }, 'Added images to thread body');
                    }
                } catch (error) {
            ...
            logger.warn({ 
                threadId: thread.id, 
                threadName: thread.name,
                hasStarterMessage: false
            }, 'No starter message found for thread');
        }
    } catch (error) {
        logger.warn({ error, threadId: thread.id, threadName: thread.name }, 'Failed to fetch starter message');
    }
